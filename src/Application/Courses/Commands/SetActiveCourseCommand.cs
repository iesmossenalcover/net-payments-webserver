...
public class SetActiveCourseCommandHandler : IRequestHandler<SetActiveCourseCommand, Response<Unit>>
{
    ...
    public async Task<Response<Unit>> Handle(SetActiveCourseCommand request, CancellationToken ct)
    {
        ...
        Course? newCurrentCourse = await _coursesRepository.GetByIdAsync(request.GetId, false, ct);
        if (newCurrentCourse == null)
            return Response<Unit>.Error(ResponseCode.NotFound, @"El curs que es vol activar no existeix.");
