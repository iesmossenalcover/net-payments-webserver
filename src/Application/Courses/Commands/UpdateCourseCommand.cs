...
{
    public long GetId { get; private set; }
...
public class UpdateCourseCommandHandler : IRequestHandler<UpdateCourseCommand, Response<Unit>>
{
    ...
    public async Task<Response<Unit>> Handle(UpdateCourseCommand request, CancellationToken ct)
    {
        if (!request.StartDate.HasValue || !request.EndDate.HasValue)
            return Response<Unit>.Error(ResponseCode.BadRequest,
                @"S'han d'especificar una data d'inici i una data de fi");

        Course? course = await _coursesRepository.GetByIdAsync(request.GetId, false, ct);

        if (course == null)
            return Response<Unit>.Error(ResponseCode.NotFound, @"El curs que es vol editar no existeix.");
