...
public class UpdateEventCommandHandler : IRequestHandler<UpdateEventCommand, Response<long?>>
{
    ...
    {
        Event? e = await _eventsRespository.GetByIdAsync(request.GetId, false, ct);
        if (e == null)
            return Response<long?>.Error(ResponseCode.NotFound, @"L'event que es vol modificar no existeix.");
        ...
        e.UnpublishDate = request.UnpublishDate.HasValue
