...
public class GetEventByIdQueryHandler : IRequestHandler<GetEventByIdQuery, Response<EventVm>>
{
    ...
    {
        Event? e = await _eventsRepository.GetByIdAsync(request.Id, true, ct);
        if (e == null)
        {
            return Response<EventVm>.Error(ResponseCode.NotFound, @"No s'ha trobat cap esdeveniment amb aquest id");
        }

        return Response<EventVm>.Ok(
            new EventVm(
                e.Id, e.Code, e.Description, e.Name, e.Price, e.AmipaPrice, e.Enrollment, e.Amipa, e.MaxQuantity,
                e.Date, e.CreationDate, e.PublishDate, e.UnpublishDate, e.IsActive
            ));
    }
}