...
public record ListEventPaymentsVm(long Id, string Code, decimal TotalPrice, decimal AmipaTotalPrice, int Count, int PaidCount, IEnumerable<EventPaymentVm> Payments);
...
public class ListEventPaymentsQueryHandler : IRequestHandler<ListEventPaymentsQuery, Response<ListEventPaymentsVm>>
{
    ...
    public async Task<Response<ListEventPaymentsVm>> Handle(ListEventPaymentsQuery request, CancellationToken ct)
    {
        ...
        var vm = new ListEventPaymentsVm(e.Id, e.Code, totalPrice, amiaPrice, eventPeople.Count(), paidCount, payments);
