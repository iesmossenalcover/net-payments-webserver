...
public record PublicEventVm(string Code, string Name, decimal Price, string DisplayPrice, bool selectable);
...
public class PersonActiveEventsQueryHandler : IRequestHandler<PersonActiveEventsQuery, Response<PersonActiveEventsVm>>
{
    ...
    public async Task<Response<PersonActiveEventsVm>> Handle(PersonActiveEventsQuery request, CancellationToken ct)
    {
        ...
        IEnumerable<PublicEventVm> eventsVm = personEvents.Select(x => new PublicEventVm(x.Event.Code, x.Event.Name, pgc.PriceForEvent(x.Event), $"{pgc.PriceForEvent(x.Event)} €", true));
