...
public class RedsysApi : IRedsys
{
    ...
    public RedsysResult GetResult(string merchantParameters)
    {
        ...
        string orderCode = merchantParametersResponse.OrderCode;
        if (!int.TryParse(merchantParametersResponse.StatusCode, out var code)) return new RedsysResult(false, orderCode, "No s'ha pogut obtenir el codi de resposta");

        if (code >= 0 && code < 100)
        {
            return new RedsysResult(true, orderCode);
        }

        return new RedsysResult(false, orderCode, merchantParametersResponse.StatusCode);
    }
